 ...
 */
public class LayerShaclServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (layer == null) {
			...
			File shaclFile = new File(layer.getShaclPath());

			if (!shaclFile.exists()) {
				// SHACL file doesn't exist, send 404 Not Found
				resp.setStatus(HttpServletResponse.SC_NOT_FOUND);
				resp.getWriter().write("SHACL file not found for the specified layer.");
			} else {
				...
				try (PrintWriter writer = resp.getWriter()) {
					...
					FileInputStream inputStream = new FileInputStream(shaclFile);
